...
export function RegisterDonor() {
    ...
    function createAcc() {
        const updatedFormData = { ...formData, role: "Donor" };

        dispatch(addUser(updatedFormData));

        console.log(updatedFormData);
        localStorage.setItem("email", updatedFormData.email);
        localStorage.setItem("password", updatedFormData.password);
        localStorage.setItem("name", updatedFormData.name);
        localStorage.setItem("role", updatedFormData.role); // Store role in localStorage
                    ...
                    <div className="flex-1 p-12">
                        ...
                        <div className="py-4 px-[10px]">
                            ...
                            <p className="mt-6 text-xs text-gray-500 text-center">
                                Have an account?{" "}
                                <span className="text-[#17502E] hover:underline cursor-pointer" onClick={login}>
                                Login now.
                            </span>
                            </p>
                        </div>
